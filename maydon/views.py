...
from .serializers import (
    ...
    FootballFieldOwnerSerializer,
    FootballFieldBronSerializer,
)
...
def api_for_stadum_owner_create(request):
    ...
    if user.is_authenticated:
        if user.role == "field_owner":
            ...
            return Response(
                {"error": "You are not the owner"}, status=status.HTTP_403_FORBIDDEN
            )
    else:
        return Response(
        ...
        if user.role == "regular_user":
            if request.method == "GET":
                ...
                try:
                    ...
                    return Response(
                        {"error": "Field not found or you are not the owner"},
    ...
    if user.is_authenticated:
        try:
            ...
            if request.method == "GET":
                ...
                serializer = FootballFieldOwnerSerializer(
            ...
            return Response(
                {"error": "Field not found or you are not the owner"}, status=404
            )
    else:
